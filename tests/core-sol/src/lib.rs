...
#[test]
fn signature_by_selector_basic() {
    alloy_core::sol! {
        contract NBSelTest {
            function foo(uint256);
            function bar(address);
    ...
    let foo_sel = <NBSelTest::fooCall as alloy_core::sol_types::SolCall>::SELECTOR;
    let bar_sel = <NBSelTest::barCall as alloy_core::sol_types::SolCall>::SELECTOR;
    assert_eq!(NBSelTest::NBSelTestCalls::signature_by_selector(foo_sel), Some("foo(uint256)"));
    assert_eq!(NBSelTest::NBSelTestCalls::signature_by_selector(bar_sel), Some("bar(address)"));
    assert_eq!(NBSelTest::NBSelTestCalls::signature_by_selector([0, 0, 0, 0]), None);

    // Errors
    let fooe_sel = <NBSelTest::FooErr as alloy_core::sol_types::SolError>::SELECTOR;
    let bare_sel = <NBSelTest::BarErr as alloy_core::sol_types::SolError>::SELECTOR;
    assert_eq!(
        NBSelTest::NBSelTestErrors::signature_by_selector(fooe_sel),
        Some("FooErr(uint256)")
    );
    assert_eq!(
        NBSelTest::NBSelTestErrors::signature_by_selector(bare_sel),
        Some("BarErr(address)")
    );
    assert_eq!(NBSelTest::NBSelTestErrors::signature_by_selector([0, 0, 0, 0]), None);
}
