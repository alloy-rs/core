...
///
/// Each entry is 64 bytes (32-byte key + 32-byte value), so 100k entries uses ~6.4MB
/// plus LRU overhead (~2MB). Total: ~8-10MB.
///
/// Override via `KECCAK_CACHE_SIZE` environment variable.
pub const DEFAULT_KECCAK_CACHE_SIZE: u32 = 100_000;

/// Environment variable name for configuring cache size.
pub const KECCAK_CACHE_SIZE_ENV: &str = "KECCAK_CACHE_SIZE";

type KeccakLruMap = LruMap<B256, B256, ByLength>;
...
impl KeccakCache {
    fn new(max_size: u32) -> Self {
        Self {
            inner: RwLock::new(LruMap::new(ByLength::new(max_size))),
...
/// Creates a cache key from the input bytes.
///
/// Returns `Some(key)` for cacheable inputs (20 or 32 bytes), `None` otherwise.
///
/// The key encoding ensures no collisions between 20-byte and 32-byte inputs:
/// - 32-byte inputs: used directly as the key
/// - 20-byte inputs: padded with `0xFF` bytes (which cannot occur in a valid 32-byte
///   input that would collide, since addresses are zero-padded when extended to 32 bytes
///   in Ethereum, not `0xFF`-padded)
#[inline]
fn make_cache_key(input: &[u8]) -> Option<B256> {
    match input.len() {
        32 => Some(B256::from_slice(input)),
        20 => {
            // Pad 20-byte addresses with 0xFF to distinguish from 32-byte inputs
            // This prevents collisions: a 32-byte key ending in 0xFF...FF would need
            // to be an address that ends with 0xFF bytes, which is astronomically unlikely
            // and would still produce correct (just uncached) results if it occurred.
            let mut key = B256::repeat_byte(0xFF);
            key[..20].copy_from_slice(input);
            Some(key)
        }
        _ => None,
    }
...
pub(crate) fn keccak256_cached(input: &[u8], compute: impl FnOnce(&[u8]) -> B256) -> B256 {
    ...
    // Only cache 20 and 32 byte inputs
    let Some(cache_key) = make_cache_key(input) else {
        cache.stats.record_bypass();
        return compute(input);
    };

    // Try cache lookup (read lock)
    {
        let map = cache.inner.read();
        if let Some(&result) = map.peek(&cache_key) {
            cache.stats.record_hit();
            return result;
        }
    }

    // Cache miss - compute and insert (write lock)
...
/// Returns the current number of entries in the keccak256 cache.
...
mod tests {
    ...
    fn test_cache_key_generation() {
        // 20-byte input should produce a cache key
        assert!(make_cache_key(&[0u8; 20]).is_some());

        // 32-byte input should produce a cache key
        assert!(make_cache_key(&[0u8; 32]).is_some());

        // Other sizes should return None (bypass)
        assert!(make_cache_key(&[0u8; 0]).is_none());
        assert!(make_cache_key(&[0u8; 19]).is_none());
        assert!(make_cache_key(&[0u8; 21]).is_none());
        assert!(make_cache_key(&[0u8; 31]).is_none());
        assert!(make_cache_key(&[0u8; 33]).is_none());
        assert!(make_cache_key(&[0u8; 100]).is_none());
    }
    ...
    #[test]
    fn test_no_collision_between_20_and_32_byte_inputs() {
        // This test ensures that a 20-byte input and a 32-byte input that
        // shares the same first 20 bytes (with zeros in the remaining 12)
        ...
        assert_ne!(hash_20, hash_32, "20-byte and 32-byte inputs with same prefix must hash differently");
