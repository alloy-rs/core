...
///
/// ```
/// use alloy_primitives::map::rand::RandMap;
///
...
impl<K, V, S> RandMap<K, V, S> {
    ...
    pub const fn inner_mut(&mut self) -> &mut IndexMap<K, V, S> {
        &mut self.inner
    }
    ...
    S: BuildHasher,
{
    ...
    /// Removes a key from the map, returning the value if the key was previously in the
    /// map. **Uses shift-remove** (preserves insertion order at cost of O(n)).
...
#[cfg(feature = "serde")]
impl<K, V, S> serde::Serialize for RandMap<K, V, S>
where
...
#[cfg(feature = "serde")]
impl<'de, K, V, S> serde::Deserialize<'de> for RandMap<K, V, S>
where
    ...
    S: BuildHasher + Default,
{
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        IndexMap::deserialize(deserializer).map(|inner| Self { inner })
    }
}
...
///
/// ```
/// use alloy_primitives::map::rand::RandSet;
///
/// let mut set: RandSet<&str> = RandSet::default();
/// set.insert("a");
/// set.insert("b");
/// set.insert("c");
///
...
impl<T, S> RandSet<T, S> {
    ...
    pub const fn inner_mut(&mut self) -> &mut IndexSet<T, S> {
        &mut self.inner
    }
    ...
    S: BuildHasher,
{
    ...
    /// Removes a value from the set. Returns `true` if the value was present.
    /// **Uses shift-remove** (preserves insertion order at cost of O(n)).
...
#[cfg(feature = "serde")]
impl<T, S> serde::Serialize for RandSet<T, S>
where
...
#[cfg(feature = "serde")]
impl<'de, T, S> serde::Deserialize<'de> for RandSet<T, S>
where
    T: serde::Deserialize<'de> + Hash + Eq,
    S: BuildHasher + Default,
{
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        IndexSet::deserialize(deserializer).map(|inner| Self { inner })
    }
}
...
mod tests {
    ...
    fn two_iterations_can_differ() {
        // With 20 elements and random stride, the probability of two iterations
        // producing the identical order is astronomically low (~1/20!).
        let map: RandMap<i32, i32> = (0..20).map(|i| (i, i)).collect();

        let order1: alloc::vec::Vec<i32> = map.keys().copied().collect();
        let order2: alloc::vec::Vec<i32> = map.keys().copied().collect();

        // Run multiple attempts to account for the (tiny) chance of collision.
        let mut any_differ = order1 != order2;
        for _ in 0..10 {
            let order_n: alloc::vec::Vec<i32> = map.keys().copied().collect();
            if order_n != order1 {
                any_differ = true;
                break;
            }
    ...
    fn set_two_iterations_can_differ() {
        ...
        let order1: alloc::vec::Vec<&i32> = set.iter().collect();
        let mut any_differ = false;
        for _ in 0..10 {
            let order_n: alloc::vec::Vec<&i32> = set.iter().collect();
            if order_n != order1 {
                any_differ = true;
                break;
            }
    ...
    fn equality() {
        ...
        // IndexMap equality is order-independent for key-value pairs.
        assert_eq!(a, b);
    }
