...
impl Signature for EcdsaSignature {
    ...
    /// Creates a [`EcdsaSignature`] from the serialized `r` and `s` scalar values, which comprise
    /// the ECDSA signature, alongside a `v` value, used to determine the recovery ID.
    #[inline]
    fn from_scalars_and_parity<T: TryInto<Parity, Error = E>, E: Into<SignatureError>>(
        r: crate::B256,
        s: crate::B256,
        parity: T,
    ) -> Result<Self, SignatureError> {
    ...
    #[inline]
    fn from_bytes_and_parity<T: TryInto<Parity, Error = E>, E: Into<SignatureError>>(
        bytes: &[u8],
        parity: T,
    ) -> Result<Self, SignatureError> {
...
impl From<&EcdsaSignature> for [u8; 65] {
    #[inline]
    fn from(value: &EcdsaSignature) -> [u8; 65] {
        value.as_bytes()
    }
}
...
#[cfg(feature = "k256")]
impl From<(k256::ecdsa::Signature, k256::ecdsa::RecoveryId)> for EcdsaSignature {
...
#[cfg(feature = "k256")]
impl TryFrom<EcdsaSignature> for k256::ecdsa::Signature {
    type Error = k256::ecdsa::Error;

    fn try_from(value: EcdsaSignature) -> Result<Self, Self::Error> {
        value.to_k256()
    }
}

#[cfg(feature = "rlp")]
impl alloy_rlp::Encodable for EcdsaSignature {
...
#[cfg(feature = "rlp")]
impl alloy_rlp::Decodable for EcdsaSignature {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        impl<'de> serde::de::Visitor<'de> for MapVisitor {
                ...
                A: MapAccess<'de>,
            {
                ...
                EcdsaSignature::from_rs_and_parity(r, s, v).map_err(serde::de::Error::custom)
            }
        }
...
impl<'a> arbitrary::Arbitrary<'a> for EcdsaSignature {
...
#[cfg(feature = "arbitrary")]
impl proptest::arbitrary::Arbitrary for EcdsaSignature {
    type Parameters = ();
...
mod tests {
    ...
    fn can_recover_tx_sender_not_normalized() {
        let sig = EcdsaSignature::from_str("48b55bfa915ac795c431978d8a6a992b628d557da5ff759b307d495a36649353efffd310ac743f371de3b9f7f9cb56c0b28ad43601b4ab949f53faa07bd2c8041b").unwrap();
    ...
    fn recover_web3_signature() {
        ...
        let sig = EcdsaSignature::from_str(
    ...
    fn signature_from_str() {
        let s1 = EcdsaSignature::from_str(
        ...
        let s2 = EcdsaSignature::from_str(
    ...
    fn deserialize_without_parity() {
        ...
        let expected = EcdsaSignature::from_rs_and_parity(
    ...
    fn serialize_both_parity() {
        ...
        let signature = EcdsaSignature::from_rs_and_parity(
            U256::from_str("0xc569c92f176a3be1a6352dd5005bfc751dcb32f57623dd2a23693e64bf4447b0")
                .unwrap(),
            U256::from_str("0x1a891b566d369e79b7a66eecab1e008831e22daa15f91a0a0cf4f9f28f47ee05")
                .unwrap(),
    ...
    fn serialize_v_hex() {
        ...
        let sig = serde_json::from_str::<EcdsaSignature>(s).unwrap();
    ...
    fn test_bincode_roundtrip() {
        ...
        assert_eq!(bincode::deserialize::<EcdsaSignature>(&bin).unwrap(), signature);
    }
    ...
    fn signature_rlp_decode() {
        ...
        let result = EcdsaSignature::decode(&mut &bytes[..]).unwrap();

        // Assert that the decoded Signature matches the expected Signature
        assert_eq!(
            result,
            EcdsaSignature::from_str("48b55bfa915ac795c431978d8a6a992b628d557da5ff759b307d495a3664935310002cef538bc0c8e21c46080634a93e082408b0ad93f4a7207e63ec5463793d01").unwrap()
        );
    }
    ...
    fn test_encode_and_decode() {
        ...
        let decoded = EcdsaSignature::decode(&mut &*encoded).unwrap();
