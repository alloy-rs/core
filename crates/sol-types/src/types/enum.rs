...
pub trait SolEnum: Sized + Copy + Into<u8> + TryFrom<u8, Error = crate::Error> {
    ...
    fn abi_decode_unchecked(data: &[u8]) -> Result<Self> {
        <crate::sol_data::Uint<8> as SolType>::abi_decode_unchecked(data).and_then(Self::try_from)
    }

    /// ABI decode the enum from the given buffer, with validation.
    ///
    /// This is the same as [`abi_decode_unchecked`](Self::abi_decode_unchecked), as validation
    ...
    fn abi_decode_validate(data: &[u8]) -> Result<Self> {
        ...
        Self::abi_decode_unchecked(data)
    }
