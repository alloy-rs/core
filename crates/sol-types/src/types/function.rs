...
pub trait SolCall: Sized {
    ...
    fn abi_decode_returns_unchecked(data: &[u8]) -> Result<Self::Return>;

    /// ABI decode this call's return values from the given slice, with validation.
    ///
    /// This is the same as [`abi_decode_returns_unchecked`](Self::abi_decode_returns_unchecked), but performs
