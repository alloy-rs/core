...
pub trait SolEventInterface: Sized {
    ...
    ///
    /// Implementations generated by `sol!` reject logs matching more than one
    /// event variant. A resource-limit error while checking a candidate is also
    /// returned, since the log's event type cannot then be determined uniquely.
