...
pub trait TokenType<'a>: Sealed + Sized {
...
impl<'a, T: TokenType<'a>, const N: usize> TokenType<'a> for FixedSeqToken<T, N> {
...
impl<'a, T: TokenType<'a>, const N: usize> TokenSeq<'a> for FixedSeqToken<T, N> {
    fn encode_sequence(&self, enc: &mut Encoder) {
...
impl<'a, T: TokenType<'a>> TokenType<'a> for DynSeqToken<T> {
...
impl<'a, T: TokenType<'a>> TokenSeq<'a> for DynSeqToken<T> {
    fn encode_sequence(&self, enc: &mut Encoder) {
        let head_words = self.0.iter().map(TokenType::head_words).sum::<usize>();
        enc.push_offset(head_words as u32);
        for t in self.0.iter() {
...
impl<'a> TokenType<'a> for PackedSeqToken<'a> {
    const DYNAMIC: bool = true;
...
macro_rules! tuple_impls {
    ($($ty:ident),+) => {
        impl<'aa, $($ty: TokenType<'aa>,)+> Sealed for ($($ty,)+) {}

        #[allow(non_snake_case)]
        impl<'aa, $($ty: TokenType<'aa>,)+> TokenType<'aa> for ($($ty,)+) {
            const DYNAMIC: bool = $( <$ty as TokenType>::DYNAMIC )||+;

            #[inline]
            fn decode_from(dec: &mut Decoder<'aa>) -> Result<Self> {
        ...
        #[allow(non_snake_case)]
        impl<'aa, $($ty: TokenType<'aa>,)+> TokenSeq<'aa> for ($($ty,)+) {
            ...
            fn decode_sequence(dec: &mut Decoder<'aa>) -> Result<Self> {
                Ok(($(
...
impl<'a> TokenType<'a> for () {
    const DYNAMIC: bool = false;

    #[inline]
    fn decode_from(_dec: &mut Decoder<'a>) -> Result<Self> {
        Ok(())
    }
...
impl<'a> TokenSeq<'a> for () {
    ...
    #[inline]
    fn decode_sequence(_dec: &mut Decoder<'a>) -> Result<Self> {
        Ok(())
    }
}
