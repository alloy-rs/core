...
impl SolInput {
    ...
    pub fn normalize_json(self) -> Result<Self> {
        self.normalize_json_with_config(ToSolConfig::new())
    }

    /// Normalize JSON ABI inputs into Sol inputs using the given configuration.
    pub fn normalize_json_with_config(self, config: ToSolConfig) -> Result<Self> {
        let SolInput {
            attrs,
            path,
...
fn abi_to_sol(name: &Ident, abi: &mut JsonAbi, config: ToSolConfig) -> String {
    abi.dedup();
    let config = config.print_constructors(true).for_sol_macro(true);
...
mod tests {
    ...
    fn parse_test(s: &str, path: &str) {
        ...
        let name_id = id(name);
        let sol = abi_to_sol(&name_id, &mut abi, ToSolConfig::new());
