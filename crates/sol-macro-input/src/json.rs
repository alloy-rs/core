...
use alloy_json_abi::{ContractObject, EnumDefinitions, JsonAbi, ToSolConfig};
...
impl SolInput {
    ...
    pub fn normalize_json(self) -> Result<Self> {
        self.normalize_json_with_enums(&EnumDefinitions::new())
    }

    /// Normalize JSON ABI inputs into Sol inputs using known enum definitions.
    pub fn normalize_json_with_enums(self, enums: &EnumDefinitions) -> Result<Self> {
        let SolInput {
            attrs,
            path,
        ...
        let sol = abi_to_sol(&name, &mut abi, enums);
...
fn abi_to_sol(name: &Ident, abi: &mut JsonAbi, enums: &EnumDefinitions) -> String {
    abi.dedup();
    let config = ToSolConfig::new().print_constructors(true).for_sol_macro(true);
    abi.to_sol_with_enums(&name.to_string(), Some(config), enums)
}
...
mod tests {
    ...
    #[test]
    fn normalize_json_with_enum_definitions() {
        let abi = serde_json::from_str::<JsonAbi>(
            r#"[{
                ...
                "inputs": [
                    {"name": "status", "type": "uint8", "internalType": "enum C.Status"}
                ],
                "outputs": []
            }]"#,
        )
        ...
        let enums =
            EnumDefinitions::from([("C.Status".into(), vec!["Pending".into(), "Active".into()])]);

        let normalized = input.normalize_json_with_enums(&enums).unwrap();
        let SolInputKind::Sol(file) = normalized.kind else { panic!("expected Solidity input") };
        let enum_item = file.items.iter().find_map(|item| {
            let ast::Item::Contract(contract) = item else { return None };
            contract.body.iter().find_map(|item| {
                let ast::Item::Enum(item) = item else { return None };
                Some(item)
            })
        });
        let enum_item = enum_item.expect("expected normalized enum declaration");
        assert_eq!(enum_item.name.to_string(), "Status");
        assert_eq!(
            enum_item.variants.iter().map(ToString::to_string).collect::<Vec<_>>(),
            ["Pending", "Active"]
        );
    }
    ...
    fn parse_test(s: &str, path: &str) {
        ...
        let name_id = id(name);
        let sol = abi_to_sol(&name_id, &mut abi, &EnumDefinitions::new());
