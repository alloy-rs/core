//! # ethers-sol-type
//!
//! This crate provides the [`sol`][sol!] procedural macro, which parses Solidity syntax to generate
//! types that implement [`ethers-abi-enc`] traits.
//!
...
/// Parses Solidity syntax to generate types that implement [`ethers-abi-enc`] traits.
///
/// These types may then be used for safe [ABI] and [EIP-712] encoding and decoding.
///
...
/// # Examples
///
/// Note: the following examples cannot tested here because the generated code references
/// `ethers-abi-enc`, so they are [tested in that crate][tests] and included with `include_str!`
/// in this doc instead.
///
