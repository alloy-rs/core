...
pub struct DefaultFrame {
    pub failed: bool,
    pub gas: u64,
    pub return_value: Bytes, // TODO: serialize_with no prefix
...
pub struct StructLog {
    ...
    /// Storage slots of current contract read from and written to. Only emitted
    /// for SLOAD and SSTORE. Disabled via disableStorage
    #[serde(
        default,
        skip_serializing_if = "Option::is_none",
...
pub enum GethDebugBuiltInTracerType {
    ...
    FourByteTracer,
    /// The callTracer tracks all the call frames executed during a transaction,
    /// including depth 0. The result will be a nested list of call frames,
    /// resembling how EVM works. They form a tree with the top-level call
    /// at root and sub-calls as children of the higher levels.
    ...
    /// post-state (i.e. what changed because the transaction happened). The
    /// prestateTracer defaults to prestate mode. It reexecutes the given
...
pub struct GethDebugTracingOptions {
    ...
    /// A string of decimal integers that overrides the JavaScript-based tracing
    /// calls default timeout of 5 seconds.
    #[serde(default, skip_serializing_if = "Option::is_none")]
