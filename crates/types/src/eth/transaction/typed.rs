#![allow(missing_docs)]
//! The [`TransactionRequest`][crate::TransactionRequest] is a universal
//! representation for a transaction deserialized from the json input of an RPC
//! call. Depending on what fields are set, it can be converted into the
...
impl From<TransactionKind> for crate::primitives::TransactionKind {
    fn from(kind: TransactionKind) -> Self {
        match kind {
            TransactionKind::Call(to) => crate::primitives::TransactionKind::Call(to),
            TransactionKind::Create => crate::primitives::TransactionKind::Create,
        }
    }
}
