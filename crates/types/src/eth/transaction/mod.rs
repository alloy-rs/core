...
impl Transaction {
    /// Create a new rpc transaction result for a mined transaction, using the
    /// given block hash, number, and tx index fields to populate the
    /// corresponding fields in the rpc result.
    ///
    /// The block hash, number, and tx index fields should be from the original
    ...
    /// Create a new rpc transaction result for a _pending_ signed transaction,
    /// setting block environment related fields to `None`.
