...
impl Signature {
    ...
    /// compute the signature's recovery id.
    ///
    /// If the transaction is a legacy transaction, it will use the `chain_id`
    /// to compute the signature's recovery id. If the transaction is a
    /// typed transaction, it will set the `v` value to `0` or `1` depending
