...
///
/// Note: This is a slimmed down version of [Header], primarily for
/// communicating the highest block with the P2P network and the RPC.
#[derive(
...
pub struct Header {
    ...
    /// transactions are executed and finalisations applied; formally Hr.
    pub state_root: B256,
    ...
    /// The Keccak 256-bit hash of the root node of the trie structure populated
    /// with the receipts of each transaction in the transactions list
    ...
    /// transaction in the transactions list; formally Hb.
    ...
    /// has a number of zero; formally Hi.
    pub number: BlockNumber,
    ...
    /// block’s inception; formally Hs.
    pub timestamp: u64,
    /// A 256-bit hash which, combined with the
    /// nonce, proves that a sufficient amount of computation has been carried
    /// out on this block; formally Hm.
    pub mix_hash: B256,
    /// A 64-bit value which, combined with the mixhash, proves that a
    /// sufficient amount of computation has been carried out on this block;
    /// formally Hn.
    pub nonce: u64,
    /// A scalar representing EIP1559 base fee which can move up or down each
    /// block according to a formula which is a function of gas used in
    /// parent block and gas target (block gas limit divided by elasticity
    /// multiplier) of parent block. The algorithm results in the base fee
    /// per gas increasing when blocks are above the gas target, and
    /// decreasing when blocks are below the gas target. The base fee per
    /// gas is burned.
    pub base_fee_per_gas: Option<u64>,
    /// An arbitrary byte array containing data relevant to this block. This
    /// must be 32 bytes or fewer; formally Hx.
...
impl Header {
    ...
    /// Heavy function that will calculate hash of data and will *not* save the
    /// change to metadata. Use [`Header::seal`], [`SealedHeader`] and
    /// unlock if you need hash to be persistent.
    ...
    ///
    /// WARNING: This method does not perform validation whether the hash is
    /// correct.
    ...
    fn header_payload_length(&self) -> usize {
        ...
        length += B64::from(self.nonce.to_be_bytes()).length();
...
impl Encodable for Header {
    fn encode(&self, out: &mut dyn BufMut) {
        ...
        B64::from(self.nonce.to_be_bytes()).encode(out);
...
/// [`SealedHeader::unseal()`] if you want to modify header.
...
impl proptest::arbitrary::Arbitrary for SealedHeader {
    ...
    fn arbitrary_with(_: Self::Parameters) -> Self::Strategy {
        ...
        any::<(Header, BlockHash)>()
            .prop_map(move |(header, _)| header.seal_slow())
            .boxed()
    }
}
...
/// Represents the direction for a headers request depending on the `reverse`
/// field of the request.
/// > The response must contain a number of block headers, of rising number when
/// > reverse is 0,
/// > falling when 1
///
...
mod tests {
    ...
    fn test_encode_block_header() {
        let expected = "";
        let header = Header {
