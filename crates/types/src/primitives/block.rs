...
/// Ethereum full block.
///
/// Withdrawals can be optionally included at the end of the RLP encoded
/// message.
#[derive(
...
impl BlockWithSenders {
    /// New block with senders. Return none if len of tx and senders does not
    /// match
...
impl SealedBlock {
    /// Create a new sealed block instance using the sealed header and block
    /// body.
...
impl SealedBlockWithSenders {
    /// New sealed block with sender. Return none if len of tx and senders does
    /// not match
...
impl Decodable for BlockHashOrNumber {
    fn decode(buf: &mut &[u8]) -> Result<Self, DecodeError> {
        ...
        if header == 0xa0 {
            ...
            // a block number when encoded as bytes ranges from 0 to any number of bytes -
            // we're going to accept numbers which fit in less than 64 bytes.
            // Any data larger than this which is not caught by the Hash decoding should
            // error and is considered an invalid block number.
        ...
        let block = match s {
            ...
            _number => {
                if let Some(hex_val) = s.strip_prefix("0x") {
                    BlockNumberOrTag::Number(u64::from_str_radix(hex_val, 16)?)
                } else {
                    return Err(HexStringMissingPrefixError.into())
                }
            }
        };
        Ok(block)
...
mod test {
    ...
    fn serde_blocknumber_non_0xprefix() {
        ...
        assert_eq!(err.to_string(), HexStringMissingPrefixError.to_string());
    }
}
