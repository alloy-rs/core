...
/// A component used to construct the root hash of the trie. The primary purpose
/// of a Hash Builder is to build the Merkle proof that is essential for
/// verifying the integrity and authenticity of the trie's contents. It achieves
/// this by constructing the root hash from the hashes of child nodes according
/// to specific rules, depending on the type of the node (branch, extension, or
/// leaf).
///
/// Here's an overview of how the Hash Builder works for each type of node:
///  * Branch Node: The Hash Builder combines the hashes of all the child nodes
///    of the branch node, using a cryptographic hash function like SHA-256. The
///    child nodes' hashes are concatenated and hashed, and the result is
///    considered the hash of the branch node. The process is repeated
///    recursively until the root hash is obtained.
///  * Extension Node: In the case of an extension node, the Hash Builder first
///    encodes the node's shared nibble path, followed by the hash of the next
///    child node. It concatenates these values and then computes the hash of
///    the resulting data, which represents the hash of the extension node.
///  * Leaf Node: For a leaf node, the Hash Builder first encodes the key-path
///    and the value of the leaf node. It then concatenates the encoded key-path
///    and value, and computes the hash of this concatenated data, which
///    represents the hash of the leaf node.
///
/// The Hash Builder operates recursively, starting from the bottom of the trie
/// and working its way up, combining the hashes of child nodes and ultimately
/// generating the root hash. The root hash can then be used to verify the
/// integrity and authenticity of the trie's data by constructing and
...
impl HashBuilder {
    ...
    /// Given a new element, it appends it to the stack and proceeds to loop
    /// through the stack state and convert the nodes it can into branch /
    /// extension nodes and hash them. This ensures that the top of the
    /// stack always contains the merkle root corresponding to the trie
    /// built so far.
    ...
    /// Given the size of the longest common prefix, it proceeds to create a
    /// branch node from the state mask and existing stack state, and store
    /// its RLP to the top of the stack, after popping all the relevant
    ...
    /// Given the current nibble prefix and the highest common prefix length,
    /// proceeds to update the masks for the next level and store the branch
    /// node and the masks in the database. We will use that when consuming
    /// the intermediate nodes from the database to efficiently build the
    /// trie.
...
mod tests {
    ...
    // Hashes the keys, RLP encodes the values, compares the trie builder with the
    // upstream root.
    ...
    fn test_generates_branch_node() {
        ...
        // We have 1 branch node update to be stored at 0x01, indicated by the first
        // nibble. That branch root node has 2 branch node children present at
        // 0x1 and 0x2.
        ...
        // 1. State Mask: 0b111. The children of the branch + the branch value at `0`,
        // `1` and `2`. 2. Hash Mask: 0b110. Of the above items, `1` and `2`
        // correspond to sub-branch nodes. 3. Tree Mask: 0b000.
        ...
        // This basically means that every BranchNodeCompact is capable of storing up to
        // 2 levels deep of nodes (?).
    ...
    fn manual_branch_node_ok() {
        ...
        // We set this to `4` and `7` because that mathces the 2nd element of the
        // corresponding leaves. We set this to `7` because the 2nd element of
        // Leaf 1 is `7`.
        branch[4] = &leaf1;
        branch[7] = &leaf2;
