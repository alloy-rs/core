...
/// A leaf node represents the endpoint or terminal node in the trie. In other
/// words, a leaf node is where actual values are stored.
///
/// A leaf node consists of two parts: the key (or path) and the value. The key
/// is typically the remaining portion of the key after following the path
/// through the trie, and the value is the data associated with the full key.
/// When searching the trie for a specific key, reaching a leaf node means that
/// the search has successfully found the value associated with that key.
#[derive(Default)]
...
impl<'a> LeafNode<'a> {
    ...
    /// RLP(keccak(RLP(node))) depending on if the serialized node was
    /// longer than a keccak).
...
impl Encodable for LeafNode<'_> {
    fn encode(&self, out: &mut dyn BufMut) {
        #[derive(ethers_rlp::RlpEncodable)]
        struct S<'a> {
            encoded_path: &'a [u8],
            value: &'a [u8],
        }
        S {
            encoded_path: &self.key,
            value: self.value,
        }
        .encode(out);
    }
