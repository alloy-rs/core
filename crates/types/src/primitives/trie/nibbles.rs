...
impl std::fmt::Debug for Nibbles {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Nibbles")
            .field("hex_data", &hex::encode(&self.hex_data))
            .finish()
    }
}
...
impl Nibbles {
    ...
    /// Take a byte array (slice or vector) as input and convert it into a
    /// [Nibbles] struct containing the nibbles (half-bytes or 4 bits) that
    /// make up the input byte data.
    pub fn unpack<T: AsRef<[u8]>>(data: T) -> Self {
        Nibbles {
            hex_data: data
                .as_ref()
                .iter()
                .flat_map(|item| [item / 16, item % 16])
                .collect(),
        }
    }
    ...
    ///
    /// This method combines each pair of consecutive nibbles into a single
    /// byte, effectively reducing the size of the data by a factor of two.
    /// If the number of nibbles is odd, the last nibble is shifted left by 4
    /// bits and added to the packed byte vector.
    ...
    /// Encodes a given path leaf as a compact array of bytes, where each byte
    /// represents two "nibbles" (half-bytes or 4 bits) of the original hex
    /// data, along with additional information about the leaf itself.
    ///
    /// The method takes the following input:
    /// `is_leaf`: A boolean value indicating whether the current node is a leaf
    /// node or not.
    ///
    /// The first byte of the encoded vector is set based on the `is_leaf` flag
    /// and the parity of the hex data length (even or odd number of
    /// nibbles).
    ///  - If the node is an extension with even length, the header byte is
    ///    `0x00`.
    ///  - If the node is an extension with odd length, the header byte is `0x10
    ///    + <first nibble>`.
    ///  - If the node is a leaf with even length, the header byte is `0x20`.
    ///  - If the node is a leaf with odd length, the header byte is `0x30 +
    ///    <first nibble>`.
    ///
    /// If there is an odd number of nibbles, store the first nibble in the
    /// lower 4 bits of the first byte of encoded.
    ///
    /// # Returns
    ///
    /// A `Vec<u8>` containing the compact byte representation of the nibble
    /// sequence, including the header byte.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// The last element of the hex vector is used to determine whether the
    /// nibble sequence represents a leaf or an extension node. If the last
    /// element is 0x10 (16), then it's a leaf.
    pub fn is_leaf(&self) -> bool {
        self.hex_data[self.hex_data.len() - 1] == 16
    }

    /// Returns `true` if the current nibble sequence starts with the given
    /// prefix.
    ...
    /// Returns the length of the common prefix between the current nibble
    /// sequence and the given.
