...
use ethers_primitives::{keccak256, Address, Bytes, FixedBytes, B256};
...
pub const EMPTY_ROOT: B256 = FixedBytes(hex!(
...
pub fn calculate_log_root(logs: &[Log]) -> B256 {
    ...
    ethers_rlp::encode_list(logs, &mut logs_rlp);
    keccak256(logs_rlp)
}
...
pub fn calculate_ommers_root(ommers: &[Header]) -> B256 {
    ...
    ethers_rlp::encode_list(ommers, &mut ommers_rlp);
...
mod tests {
    ...
    use ethers_primitives::{Address, Bloom, B256, U256};
