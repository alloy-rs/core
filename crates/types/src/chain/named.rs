...
//   2. add extra information in the last `impl` block (explorer URLs, block
//      time) when applicable;
//   3. (optional) add aliases:
//     - Strum (in kebab-case):
// `#[strum(to_string = "<main>", serialize = "<aliasX>", ...)]`
//      `to_string = "<main>"` must be present and will be used in `Display`,
//      `Serialize`  and `FromStr`, while `serialize = "<aliasX>"` will be
//      appended to `FromStr`.
...
// We don't derive Serialize because it is manually implemented using AsRef<str>
// and it would break a lot of things since Serialize is `kebab-case` vs
// Deserialize `snake_case`. This means that the NamedChain type is not
// "round-trippable", because the Serialize and Deserialize implementations do
// not use the same case style.
...
// This must be implemented manually so we avoid a conflict with
...
macro_rules! impl_into_numeric {
    ($($ty:ty)+) => {$(
        impl From<NamedChain> for $ty {
            #[inline]
            fn from(chain: NamedChain) -> Self {
                <$ty>::from(chain as u64)
            }
        }
    )+};
}
...
impl_into_numeric!(u128 U64 U128 U256);

impl<const BITS: usize, const LIMBS: usize> TryFrom<Uint<BITS, LIMBS>> for NamedChain {
    type Error = ParseChainError;

    fn try_from(value: Uint<BITS, LIMBS>) -> Result<Self, Self::Error> {
        match LIMBS {
            0 => Err(ParseChainError { number: 0 }),
...
impl NamedChain {
    ...
    /// of an HTTP provider for example.
    ///
    /// **Note:** this is not an accurate average, but is rather a sensible
    /// default derived from blocktime charts such as [Etherscan's](https://etherscan.com/chart/blocktime)
    ...
    ///
    /// assert_eq!(
    ///     NamedChain::Mainnet.average_blocktime_hint(),
    ...
    /// Returns whether the chain implements EIP-1559 (with the type 2 EIP-2718
    /// transaction type).
    ///
    /// # Examples
    ///
    ...
    pub const fn etherscan_urls(self) -> Option<(&'static str, &'static str)> {
        ...
        let urls = match self {
            ...
            Chiado => (
                "https://blockscout.chiadochain.net/api",
            ...
            Canto => (
                "https://evm.explorer.canto.io/api",
            ...
            AnvilHardhat | Dev | Morden | MoonbeamDev | FilecoinMainnet => {
                // this is explicitly exhaustive so we don't forget to add new urls when adding
                // a new chain
                return None
            }
        };
    ...
    /// Returns the chain's blockchain explorer's API key environment variable's
    /// default name.
    ///
    /// # Examples
    ///
    ...
    /// Returns the chain's blockchain explorer's API key, from the environment
    /// variable with the name specified in
    /// [`etherscan_api_key_name`](NamedChain::etherscan_api_key_name).
    ///
    /// # Examples
    ///
    /// ```
...
mod tests {
    ...
    fn default() {
        assert_eq!(
            serde_json::to_string(&NamedChain::default()).unwrap(),
            "\"mainnet\""
        );
    }
    ...
    fn roundtrip_serde() {
        for chain in NamedChain::iter() {
            ...
            assert_eq!(
                serde_json::from_str::<'_, NamedChain>(&chain_string).unwrap(),
                chain
            );
        }
    }
    ...
    fn aliases() {
        ...
        const ALIASES: &[(NamedChain, &[&str])] = &[
            ...
            (
                PolygonZkEvmTestnet,
                &["zkevm-testnet", "polygon-zkevm-testnet"],
            ),
