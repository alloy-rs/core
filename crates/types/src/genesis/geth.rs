...
/// This represents the chain configuration, specifying the genesis block,
/// header fields, and hard fork switch blocks.
...
impl Genesis {
    ...
    pub fn new(chain_id: u64, signer_addr: Address) -> Genesis {
        // set up a clique config with an instant sealing period and short (8 block)
        // epoch
        let clique_config = CliqueConfig {
            period: Some(U64::ZERO),
            epoch: Some(U64::from(8_u64)),
        };

        let config = ChainConfig {
            chain_id,
            eip155_block: Some(U64::ZERO),
            eip150_block: Some(U64::ZERO),
            eip158_block: Some(U64::ZERO),

            homestead_block: Some(U64::ZERO),
            byzantium_block: Some(U64::ZERO),
            constantinople_block: Some(U64::ZERO),
            petersburg_block: Some(U64::ZERO),
            istanbul_block: Some(U64::ZERO),
            muir_glacier_block: Some(U64::ZERO),
            berlin_block: Some(U64::ZERO),
            london_block: Some(U64::ZERO),
...
///
/// See [geth's `ChainConfig` struct][geth-chainconfig]
/// for the source of each field.
///
/// [geth-chainconfig]: https://github.com/ethereum/go-ethereum/blob/64dccf7aa411c5c7cd36090c3d9b9892945ae813/params/config.go#L349
...
pub struct ChainConfig {
    ...
    /// A flag specifying that the network already passed the terminal total
    /// difficulty. Its purpose is to disable legacy sync without having
    /// seen the TTD locally.
...
mod tests {
    ...
    fn parse_non_hex_prefixed_balance() {
        // tests that we can parse balance / difficulty fields that are either hex or
        // decimal
        ...
        let dec_balance = "1000000000000000000000000000".parse::<U256>().unwrap();
    ...
    fn parse_hive_rpc_genesis_full() {
        ...
        let storage = alloc_entry
            .storage
            .as_ref()
            .expect("missing storage for parsed genesis");
        ...
        let code = alloc_entry
            .code
            .as_ref()
            .expect("missing code for parsed genesis");
